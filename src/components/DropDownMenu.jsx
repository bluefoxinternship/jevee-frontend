...
function DropDownMenu({ closeMenu }) {
  ...
  return (
    <div className="inset-0 w-full h-[100vh] flex flex-col bg-white">
      ...
      <div className="px-4 py-4 w-full flex">
        ...
        <div
          className={`px-[10px] py-[10px] w-[50%] flex gap-2 items-center ${
            active === "brands"
      ...
      {active === "categories" && (
        <div className="overflow-y-auto pb-10 px-[10px] pt-[10px]">
          <h2 className="font-semibold text-lg text-[#848FFC] mb-4 ">
            Shop by categories
          </h2>
          {categories.map((category, index) => {
            return (
              <ul key={index} className="flex flex-col px-2 cursor-pointer">
                <li className={`flex justify-between items-center font-semibold py-5 border-b-[1.5px] ${showCategory === category.name ? "text-[#F25CA8]":""}`} onClick={()=>setShowCategory(category.name)}>
                  {category.name} <FaAngleDown />
                  </li>

                 { showCategory === category.name && category.subCategories.map((subcategory, index) => {
                    return (
                    <ul key={index} className="flex flex-col px-[10px] cursor-pointer w-full">
                      <li><div className="border-b-[1.5px] flex justify-between items-center py-5" onClick={()=>setShowProducts(subcategory.name)}>{subcategory.name} <FaAngleDown /> </div>
                      </li>

                      {
                       showProducts === subcategory.name && subcategory.products.map((product,index)=>{
                          return (
                            <ul key={index} className="flex flex-col px-[10px] cursor-pointer w-full border-b-[1.5px]">
                              <li className="flex w-full justify-between items-center py-5">{product}</li>
                            </ul>
                          )
                        })
                      }
